...
| — | Flag a change that a **sibling repo owns** per [`plow-hermes-agent` README § The repos](https://github.com/plow-pbc/plow-hermes-agent#the-repos): the seeding and deploy mechanism (`agent-mgr` — `lib/fetch-tree` + `replay_skills()` off `skills.tsv`), container lifecycle (`agent-mgr`; this repo only declares `AGENT_LIVE=1`), and config keys the base already seeds (`plow-hermes-agent` `image/seed/config.yaml` — plugin enablement, `platforms.plow_chat`, the relay `mcp_servers` block). The test is who else would have to change if the fact changed. |
